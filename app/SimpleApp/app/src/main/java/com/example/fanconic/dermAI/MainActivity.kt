...
class MainActivity : AppCompatActivity() {
    ...
    private fun showPictureDialog() {
        ...
        var items = pictureDialog.setItems(pictureDialogItems
        ) { dialog, which ->
            when (which) {
                0 -> Companion.choosePhotoFromGallery(this)
    ...
    companion object {
        private fun choosePhotoFromGallery(mainActivity: MainActivity) {
            val galleryIntent = Intent(Intent.ACTION_PICK, MediaStore.Images.Media.EXTERNAL_CONTENT_URI)
            mainActivity.startActivityForResult(galleryIntent, mainActivity.GALLERY)
        }
    }
